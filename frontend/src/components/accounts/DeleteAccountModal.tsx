...
import { AlertTriangle, Trash2, ArrowRightLeft, CreditCard, Repeat } from "lucide-react";
...
export function DeleteAccountModal({ isOpen, onClose, account }: DeleteAccountModalProps) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      // ✨ CORRIGIDO: Zero `any`, com inferência segura
      const err = error as ApiError;
      
      if (err.response?.status === 409 && err.response.data?.conflicts) {
        setConflicts(err.response.data.conflicts);
      } else {
        alert(err.response?.data?.message || "Ocorreu um erro ao excluir a conta.");
      }
    } finally {
      ...
      <ModalBody className="p-6">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm text-muted mt-2">
            Você está prestes a excluir a conta <strong className="text-primary">{account.name}</strong>.
          </p>
        </div>
        ...
        <div className="mt-6">
          {conflicts ? (
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-5 text-left animate-in fade-in zoom-in-95">
              <h3 className="text-amber-600 font-bold text-sm mb-3">Exclusão Bloqueada</h3>
              <p className="text-secondary text-xs font-medium mb-4">
                Para preservar o seu histórico, remova ou transfira os seguintes vínculos antes de excluir esta conta:
              </p>
              <ul className="space-y-2.5">
                {conflicts.transactions > 0 && (
                  <li className="flex items-center gap-2.5 text-xs text-amber-700 font-bold bg-amber-500/10 px-3 py-2 rounded-xl">
                    <ArrowRightLeft size={16} /> {conflicts.transactions} lançamentos no caixa
                  </li>
                )}
                {conflicts.cards > 0 && (
                  <li className="flex items-center gap-2.5 text-xs text-amber-700 font-bold bg-amber-500/10 px-3 py-2 rounded-xl">
                    <CreditCard size={16} /> {conflicts.cards} cartões de crédito associados
                  </li>
                )}
                {conflicts.subscriptions > 0 && (
                  <li className="flex items-center gap-2.5 text-xs text-amber-700 font-bold bg-amber-500/10 px-3 py-2 rounded-xl">
                    <Repeat size={16} /> {conflicts.subscriptions} assinaturas vinculadas
                  </li>
                )}
              </ul>
            </div>
            ...
            <>
              <div className="bg-amber-500/10 text-amber-600 border border-amber-500/20 text-xs p-4 rounded-xl font-semibold mb-6 text-center">
                Esta ação é irreversível e só será permitida se não houverem históricos atrelados.
              </div>

              <label className="block text-sm font-medium text-primary mb-2 text-center">
                Para confirmar, digite <strong className="text-danger select-none">EXCLUIR</strong> abaixo:
              </label>
              <input
                type="text"
                placeholder="EXCLUIR"
