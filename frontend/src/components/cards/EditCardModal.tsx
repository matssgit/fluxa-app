...
// ✨ A PALAVRA "export" AQUI É O QUE FAZ O APP FUNCIONAR
export function EditCardModal({ isOpen, onClose, card }: EditCardModalProps) {
  ...
  const selectedDueDay = useWatch({ control, name: "due_day" });
        ...
        <div className="bg-surface rounded-3xl shadow-2xl w-full max-w-md max-h-[82vh] sm:max-h-[88vh] flex flex-col overflow-hidden border border-subtle/30 transition-all duration-300">
          <div className="flex justify-between items-center p-4 sm:p-6 border-b border-subtle/20 shrink-0">
            <div>
              ...
              <p className="text-xs font-medium text-muted mt-0.5 hidden sm:block">
                Atualize limites, fechamento e identidade visual
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          >
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Ex: Nubank Black, XP Infinite..."
                {...register("name")}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
              <div>
                ...
                >
                  <span>{selectedBrand || "Selecione..."}</span>
          ...
          <div className="p-4 sm:p-6 bg-surface border-t border-subtle/20 flex gap-3 shrink-0">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-2.5 sm:py-3 rounded-xl border border-subtle/30 bg-elevated hover:bg-subtle/40 text-secondary text-xs sm:text-sm font-bold transition-all shadow-2xs"
            >
              Cancelar
            </button>
            <button
              ...
              form="edit-card-form"
              disabled={isPending}
              className="flex-1 px-4 py-2.5 sm:py-3 rounded-xl bg-brand hover:bg-brand-light text-white text-xs sm:text-sm font-bold shadow-sm transition-all disabled:opacity-50"
            >
