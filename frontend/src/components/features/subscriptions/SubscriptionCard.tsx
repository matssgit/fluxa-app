...
export function SubscriptionCard({
  ...
  function handleStatusChange(newStatus: "active" | "paused"): void {
    ...
    >
      ...
      <div className="pt-3 flex items-center justify-between gap-2 mt-2">
        ...
        <div className="flex items-center gap-1 shrink-0 bg-elevated/60 p-1 rounded-xl border border-subtle/20">
          ...
          {/* Ocultar do painel apenas para canceladas */}
          {status === "cancelled" && onToggleHide && (
            <button
              onClick={() => onToggleHide(subscription.id)}
              title={isHidden ? "Desocultar assinatura" : "Ocultar do painel"}
              className="p-1.5 text-muted hover:text-primary hover:bg-surface rounded-lg transition-all cursor-pointer"
            >
